    ...
    try {
        ...
        if(userData){
            ...
            if(userData.is_verified == "1"){
                ...
                res.render('login',{message:"Email Not Verified. Please verify your email."});
            }
    ...
    try {
        ...
        if(req.body.pass == req.body.repass){
            ...
            if(userData){
                ...
                res.render('login',{message:"Registration Successfull. Verification link sent on registered email."});
            }
            else{
...
const subscribe = async(req,res)=>{
    try {
        ...
        const userUpdate = await User.findByIdAndUpdate({_id:req.body.channeluser} , { $set:{subscribers:aaa}});
...
const unsubscribe = async(req,res)=>{
    try {
        ...
        if(userUpdate){
            ...
            const subscriberData = await Subscriber.deleteOne({channeluserid:req.body.channeluser},{subscriberid:subscriberid});
...
const changepassword = async(req,res)=>{
    ...
    try {
        if(req.body.pass == req.body.repass){
            ...
            const updatedinfo = await User.updateOne( {_id:req.body.id }, { $set:{password:spassword}} );
            ...
            res.render('changepassword',{message:"Password Not Matched."},{user:user});
        }
...
const edituser = async(req,res)=>{
    try {
        ...
        const updatedinfo = await User.findByIdAndUpdate({ _id: userid },{$set:{name:req.body.name,phone:req.body.phone,description:req.body.description}});
...
const searchuser = async(req,res)=>{
    try {
        ...
        if(typeof token === "undefined"){
            const sub = "1";
            res.render('user',{ user:userData , flag:"0" , channelData:channelData ,channelCount:channelCount , sub:sub});
        }
        else{
            const verifyUser = jwt.verify(token, config.jwtSecret);
            const subscriberid = verifyUser.user_id;
...
const changeprofilepic = async(req,res)=>{
    try {
        ...
        const updatedinfo = await User.updateOne( {_id:userid}, { $set:{profileimage:req.file.filename}} );
...
const changecoverpic = async(req,res)=>{
    try {
        ...
        const updatedinfo = await User.updateOne( {_id:userid}, { $set:{coverimage:req.file.filename}} );
